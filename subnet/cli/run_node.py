...
# When LOG_JSON=true, attach a JsonFormatter to the three operational loggers.
# propagate=False is critical to prevent double-logging via the root handler —
# without it, the root basicConfig handler also fires, producing non-JSON lines
# that break `docker compose logs validator | jq` pipelines.
