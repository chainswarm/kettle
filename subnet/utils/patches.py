...
def patch_pubsub_run():
    ...
    async def _periodic_connection_sweep(self: Pubsub) -> None:
        ...
        Also detects peers that are in self.peers but have dead/closed streams
        (common on cross-CVM connections where the initial stream setup races
        with DHT peer discovery).
        """
        while True:
            ...
            for peer_id in list(network.connections.keys()):
                if self.is_peer_blacklisted(peer_id):
                    continue

                if peer_id not in self.peers:
                    # Peer connected at Swarm level but not in PubSub
                    logger.info(f"Periodic sweep: new peer {peer_id}, adding to pubsub")
                    self.manager.run_task(self._handle_new_peer_safe, peer_id)
                else:
                    # Peer in PubSub — check if stream is alive
                    stream = self.peers.get(peer_id)
                    if stream is None:
                        logger.info(f"Periodic sweep: peer {peer_id} has no stream, re-adding")
                        del self.peers[peer_id]
                        self.manager.run_task(self._handle_new_peer_safe, peer_id)
                    elif hasattr(stream, "muxed_conn") and getattr(stream.muxed_conn, "is_closed", False):
                        logger.info(f"Periodic sweep: peer {peer_id} stream closed, re-adding")
                        del self.peers[peer_id]
                        self.manager.run_task(self._handle_new_peer_safe, peer_id)
