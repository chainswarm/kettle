...
class RocksDB:
    ...
    def __init__(self, base_path: str | None = None, read_only: bool = False, secondary: bool = False):
        assert base_path is not None, "Path must be specified"
        self.base_path = base_path
        self.db_path = f"{base_path}_store"
        if secondary:
            # Secondary instance: reads from primary's DB without locking.
            # Needs its own path for catch-up state.
            from rocksdict import AccessType
            secondary_path = f"{base_path}_secondary"
            os.makedirs(secondary_path, exist_ok=True)
            self.store = Rdict(self.db_path, access_type=AccessType.secondary(secondary_path))
        elif read_only:
            from rocksdict import AccessType
